...
namespace WavRecorder
{
    ...
    public class RiffParser
    {
        ...
        public unsafe void OpenFile(string filename)
        {
            // Sanity check
            if (null != m_stream) {
                throw new Exception("RIFF file already open " + FileName);
            }
			...
			try 
			{
				...
				else
				{
					m_stream.Flush(); 
                    m_stream = null;
					throw new Exception("Error. Not a valid RIFF file " + FileName);
				}
			}
			catch (Exception ex)
			{
        ...
        public bool ReadElement(ref int bytesleft, ProcessChunkElement chunk, ProcessListElement list)
        {
            ...
            if (bytesleft < size)
            {
                ...
                SkipData(bytesleft);
                bytesleft = 0;
                throw new Exception("Element size mismatch for element " + FromFourCC(FourCC)
        ...
        public unsafe void ReadTwoInts(out int FourCC, out int size)
        {
            try {
                ...
                if (TWODWORDSSIZE != readsize) {
                    throw new Exception("Unable to read. Corrupt RIFF file " + FileName);
                }
            ...
            {
                throw new Exception("Problem accessing RIFF file " + FileName, ex);
            }
        }
        ...
        public void SkipData(int skipBytes)
        {
            ...
            {
                throw new Exception("Problem seeking in file " + FileName, ex);
            }
        }
        ...
        public int ReadData(Byte[] data, int offset, int length)
        {
            ...
            {
                throw new Exception("Problem reading data in file " + FileName, ex);
            }
        }
